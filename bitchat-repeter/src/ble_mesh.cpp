...
void BLEMesh::process() {
    ...
    if (now - lastUpdate > 10000) { // Every 10 seconds
        ...
        Serial.printf("BLE Mesh: Connected clients: %d\n", pServer->getConnectedCount());
    }
}
...
void BLEMesh::sendData(const uint8_t* data, size_t length, const String& targetPeerID) {
    if (pCharacteristic && pServer->getConnectedCount() > 0) {
        // Set the characteristic value and notify all connected clients
        pCharacteristic->setValue(data, length);
        pCharacteristic->notify();
        
        Serial.printf("BLE Mesh: Sent %d bytes to %d connected client(s)\n", 
...
void BLEMesh::handleReceivedData(const uint8_t* data, size_t length) {
    Serial.printf("BLE Mesh: Received %d bytes\n", length);
